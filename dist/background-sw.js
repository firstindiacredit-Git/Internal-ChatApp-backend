...
self.addEventListener('push', event => {
  ...
  event.waitUntil(
    self.registration.showNotification('Internal Chat', options)
  );
});
...
async function handleBackgroundNotifications() {
  try {
    ...
    for (const notification of notifications) {
      if (now - notification.timestamp > 30000) { // 30 seconds old
        notification.close();
      }
    }
...
async function cacheMessage(message) {
  try {
    ...
    await cache.put(`/api/messages/${message.id}`, new Response(JSON.stringify(message)));
  } catch (error) {
      ...
      caches.keys().then(cacheNames => {
        return Promise.all(
          cacheNames.map(cacheName => {
            if (cacheName !== CACHE_NAME && cacheName !== API_CACHE && cacheName !== MESSAGE_CACHE) {
...
console.log('Background service worker loaded - WhatsApp-like functionality enabled');
