...
io.on("connection", (socket) => {
  ...
  // Jitsi Call handlers
  // ========================
  socket.on("jitsi-call-initiate", async (data) => {
    try {
      const {
        groupId,
        groupName,
      ...
      console.log(`🎥 Jitsi call initiate request:`, {
        groupId,
        groupName,
        callType,
        roomName,
      ...
      if (!groupId || !roomName) {
        console.error("❌ Missing groupId or roomName");
        socket.emit("jitsi-call-error", {
      ...
      console.log(
        `✅ Found group: ${group.name} with ${group.members.length} members`
      );

      // Confirm to initiator
      socket.emit("jitsi-call-initiated", {
        roomName,
        callType,
        groupId,
        groupName: group.name,
      ...
      group.members.forEach((member) => {
        ...
        if (memberId !== socket.userId) {
          ...
          if (memberSocketData) {
            console.log(`📲 Notifying member: ${member.name} (${memberId})`);
            io.to(memberSocketData.socketId).emit("incoming-jitsi-call", {
              callId: roomName,
              callType,
              groupId: group._id,
              groupName: group.name,
              groupAvatar: group.profileImage || groupAvatar,
              roomName,
              initiator: {
                id: socket.userId,
                name: socket.user.name,
            ...
            console.log(`⚠️ Member ${member.name} not online`);
          }
        }
      });

      console.log(
        `✅ Jitsi call notifications sent to ${
          group.members.length - 1
        } members`
      );
    } catch (error) {
      console.error("Jitsi call initiate error:", error);
      socket.emit("jitsi-call-error", {
        error: "Failed to initiate Jitsi call",
      });
    }
  });
